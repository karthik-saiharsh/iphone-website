import gsap from "gsap";
import { useGSAP } from "@gsap/react";
import { heroVideo, smallHeroVideo } from "../utils";
import { useState, useEffect } from "react";

const Hero = () => {
    // Adjust Video source automatically based on screen size
    const [videoSrc, setVideoSrc] = useState(window.innerWidth < 760 
        ? smallHeroVideo : heroVideo);

    useGSAP(_ => {
        gsap.to("#hero-text", {
            opacity: 1,
            delay: 2
        });

        gsap.to("#cta", {
            opacity: 1,
            y: -50,
            delay: 2
        });
    }, []);

    useEffect(_ => {

        function handleResizeVideo() {
            if(window.innerWidth < 760) setVideoSrc(smallHeroVideo);
            else setVideoSrc(heroVideo);
            console.log("I am called")
        }

        window.addEventListener('resize', handleResizeVideo);

        return _ => {window.removeEventListener('resize', handleResizeVideo)};
    }, []);

    

  return (
    <section className="w-full relative bg-black nav-height">
        <div className="h-5/6 w-full flex-center">
            <p id="hero-text" className="hero-title">
                iPhone 15 Pro
            </p>

            <div className="md:w-10/12 w-9/12">
                <video autoPlay muted playsInline={true} key={videoSrc} className="pointer-events-none">
                    <source src={videoSrc} type="video/mp4"/>
                </video>
            </div>
        </div>

        <div id="cta"
        className="flex flex-col items-center opacity-0 translate-y-20">
            <a href="#highlights" className="btn">Buy</a>
            <p className="font-normal text-xl">From ₹16000/month or at ₹85000</p>
        </div>
    </section>
  )
}

export default Hero